...
import { defaultConfig } from "@rechakra/react"
...
export const EjectCommand = new Command("eject")
  ...
  .action(async (flags: CodegenFlags) => {
    ...
    import { defineGlobalStyles } from "@rechakra/react"
    ...
    import { defineLayerStyles } from "@rechakra/react"
    ...
    import { defineAnimationStyles } from "@rechakra/react"
    ...
    artifacts.push({
      ...
      content: `
import { createSystem, defaultBaseConfig, defineConfig, mergeConfigs } from "@rechakra/react"
...
function generateSemanticTokenArtifacts(
  ...
  for (const [name, token] of Object.entries(semanticTokens)) {
    const content = `
    import { defineSemanticTokens } from "@rechakra/react"
...
function generateTokenArtifacts(
  ...
  for (const [name, token] of Object.entries(tokens)) {
    const content = `
    import { defineTokens } from "@rechakra/react"
