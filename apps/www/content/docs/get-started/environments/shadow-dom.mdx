...
```bash
npm i @rechakra/react @emotion/react @emotion/cache react-shadow
```
...
Using the `@rechakra/cli` you can add snippets to your project.
...
- `ChakraProvider` from `@rechakra/react` for the styling system
...
import { ChakraProvider, EnvironmentProvider } from "@rechakra/react"
