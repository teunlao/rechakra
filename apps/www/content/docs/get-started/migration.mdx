...
Install updated versions of the packages: `@rechakra/react` and
`@emotion/react`.

```bash
npm install @rechakra/react@latest @emotion/react@latest
```
...
Update the ChakraProvider import from `@rechakra/react` to the one from the
...
- **Namespaced imports:** Import components using the dot notation for more
  ...
  ```tsx
  import { Accordion } from "@rechakra/react"
...
We've removed the `@rechakra/next-js` package in favor of using the `asChild`
...
```tsx {3}
import { Button as ChakraButton, forwardRef } from "@rechakra/react"
...
```tsx {2, 4}
import { Button as ChakraButton } from "@rechakra/react"
...
Removed `@rechakra/icons` package. Prefer to use `lucide-react` or `react-icons`
instead.
