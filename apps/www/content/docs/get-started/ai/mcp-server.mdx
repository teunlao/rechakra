...
and is published at `@rechakra/react-mcp`.
...
```bash
claude mcp add chakra-ui -- npx -y @rechakra/react-mcp
```
...
```bash
claude mcp add chakra-ui --env CHAKRA_PRO_API_KEY=your_api_key_here -- npx -y @rechakra/react-mcp
```
